                 ...
                 enum keySize size)
{
    int expandedKeySize;
    int nbrRounds;
    unsigned char expandedKey[240];  // Maximum possible size for AES (size can vary based on key size)
...
{   int expandedKeySize;
    int nbrRounds;
    unsigned char expandedKey[240];  // Maximum size needed (for SIZE_32)
    unsigned char block[16];
    int i, j;
    ...
    // Calculate expanded key size
    expandedKeySize = (16 * (nbrRounds + 1));

    // The expandedKey array is already large enough for the worst-case scenario (SIZE_32)
    if (expandedKeySize > sizeof(expandedKey)) {
        return ERROR_MEMORY_ALLOCATION_FAILED;
    }

    // Copy the input block to a local block variable
    for (i = 0; i < 4; i++)
    {
        for (j = 0; j < 4; j++)
        {
    ...
    // Expand the key and perform the AES decryption
    expandKey(expandedKey, key, size, expandedKeySize);
    aes_invMain(block, expandedKey, nbrRounds);

    // Copy the decrypted block back to the output
    for (i = 0; i < 4; i++)
    {
        for (j = 0; j < 4; j++)
        {
...
/*****************Test Function***************/
int main()
{        unsigned char key[16] = { 0x54,0x68,0x69,0x73,0x20,0x69,0x73,0x20,0x74,0x68,0x65,0x20,0x6b,0x65,0x79,0x21 };
        int expandedKeySize = 176;
        unsigned char expandedKey[expandedKeySize];
        enum keySize size = SIZE_16;
        unsigned char plaintext[16] = { 0x54,0x68,0x69,0x73,0x20,0x69,0x73,0x20,0x61,0x20,0x63,0x79,0x70,0x68,0x65,0x72 };
